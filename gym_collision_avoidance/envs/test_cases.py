...
from gym_collision_avoidance.envs.dynamics.ExternalDynamics import (
    ExternalDynamics,
)
    ...
    if type(side_length) is list:
        ...
        for comp in side_length:
            if comp["num_agents"][0] <= num_agents < comp["num_agents"][1]:
                side_length = np.random.uniform(
                    comp["side_length"][0], comp["side_length"][1]
                )
        assert type(side_length) == float

    cadrl_test_case = tc.generate_rand_test_case_multi(
        num_agents, side_length, speed_bnds, radius_bnds
    )

    agents = cadrl_test_case_to_agents(
        cadrl_test_case,
        policies=policies,
...
def full_test_suite(
    ...
    cadrl_test_case = preset_testCases(
        num_agents,
        full_test_suite=True,
        vpref_constraint=vpref_constraint,
...
def full_test_suite_carrl(
    num_agents, test_case_index, seed=None, other_agent_policy_options=None
):
    cadrl_test_case = preset_testCases(
        num_agents,
        full_test_suite=True,
    ...
    other_agent_policy = other_agent_policy_options[
        test_case_index % len(other_agent_policy_options)
    ]  # dont just sample (inconsistency btwn same test_case)
    agents.append(
        cadrl_test_case_to_agents(
            [cadrl_test_case[0, :]],
            policies="carrl",
    ...
    agents.append(
        cadrl_test_case_to_agents(
            [cadrl_test_case[1, :]],
            agents_policy=other_agent_policy,
            agents_dynamics=UnicycleDynamics,
            agents_sensors=[OtherAgentsStatesSensor],
        )[0]
    )
...
def get_testcase_random_carrl():
    ...
    speed_bnds = [0.5, 1.5]
    radius_bnds = [0.2, 0.8]
    test_case = tc.generate_rand_test_case_multi(
        num_agents, side_length, speed_bnds, radius_bnds
    )
    agents = []
    agents.append(
...
def formation(agents, letter, num_agents=6):
    ...
    for agent in agents:
        ...
        agent.reset(
            ...
            gx=goal_x,
            gy=goal_y,
            heading=agent.heading_global_frame,
        )
    return agents


def cadrl_test_case_to_agents(
    test_case,
    policies="GA3C_CADRL",
    policy_distr=None,
    agents_dynamics="unicycle",
...
def preset_testCases(
    ...
    if full_test_suite:
        ...
        if vpref_constraint:
            pref_speed_string = "vpref1.0_r{}-{}/".format(
        ...
        if seed is not None:
            filename = (
                filename[:-2] + "_seed" + str(seed).zfill(3) + filename[-2:]
            )
        ...
        if num_agents == 1:
            ...
            print(
                "[preset_testCases in Collision_Avoidance.py]                 "
                "   invalid num_agents"
            )
            assert 0
    return test_cases
...
def make_testcase_huge(
    ...
    for test_case in range(num_test_cases):
        for i in range(num_agents):
            ...
            while min_dist_to_others < 2.0:
                ...
                if i > 0:
                    min_dist_to_others = min(
                        [
                            np.linalg.norm(
                                np.array([px - tc[px_ind], py - tc[py_ind]])
                            )
                            - tc[radius_ind]
                            - radius
                            for tc in test_cases[test_case, :i, :]
                        ]
                    )
                else:
                    min_dist_to_others = np.inf
            ...
            while min_dist_to_others < 2.0 or dist_from_start < 5.0:
                ...
                if i > 0:
                    min_dist_to_others = min(
                        [
                            np.linalg.norm(
                                np.array([gx - tc[gx_ind], gy - tc[gy_ind]])
                            )
                            - tc[radius_ind]
                            - radius
                            for tc in test_cases[test_case, :i, :]
...
def get_testcase_huge():
    filename = (
        os.path.dirname(os.path.realpath(__file__)) + "/test_cases/100agents.p"
    )
    with open(filename, "rb") as f:
...
if __name__ == "__main__":
    ...
    if speed_bnds == [1.0, 1.0]:
        pref_speed_string = "vpref1.0_r{}-{}/".format(
            radius_bnds[0], radius_bnds[1]
        )
    else:
        pref_speed_string = ""
