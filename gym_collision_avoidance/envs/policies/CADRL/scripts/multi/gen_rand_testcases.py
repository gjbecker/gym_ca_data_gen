...
def distPointToSegment(p1, p2, p3):
    if np.linalg.norm(p2-p1) == float('nan'):
        sys.exit()
    # print(f'P1: {p1}')
    # print(f'P2: {p2}')
    # print(f'P3: {p3}')
    d = p2 - p1
    # print(f'd: {d}, linalg.norm: {np.linalg.norm(d)}')
    # print 'd', d
    # print '(p3-p1)', (p3-p1)
...
def generate_rand_case(num_agents, side_length, speed_bnds, radius_bnds,
    ...
    for i in range(num_agents):
        ...
        while True:
            ...
            if_collide = False
            for j in range(i):
                radius_start = test_case[j, 5] + \
                    test_case[i, 5] + GETTING_CLOSE_RANGE
                radius_end = test_case[j, 5] + \
                    test_case[i, 5] + GETTING_CLOSE_RANGE
                # start
    ...
    for i in range(num_agents):
        ...
        while min_dist_to_others < 1.0:
            ...
            if i == 0:
                ...
                min_dist_to_others = min([np.linalg.norm(
                    [start_x - other_x, start_y - other_y]) for other_x, other_y in test_case[:i, 0:2]])
    ...
    for i in range(num_agents):
        ...
        if i == 0:
            ...
            while True:
                # generate random starting/ending points
                start = (side_length * 2 *
                         np.random.rand(2,) - side_length) / 2.0
                end = start
...
def generate_swap_case(num_agents, side_length, speed_bnds, radius_bnds):
    r_min = num_agents / 1.5  # Changed 2.0 to 1.5
    r = np.random.rand() * 3.0 + r_min  # Changed 2.0 to 3.0
    test_case = np.zeros((num_agents, 6))
    counter = 0
    r_swap = 2.0 + np.random.rand() * 2.0  # Changed 1.5 to 2.0 (1.5+np.random....)
    ...
    for i in range(num_agents):
        ...
        if i == 0:
            ...
            while True:
                ...
                start = np.array(
                    [r*np.cos(start_angle), r*np.sin(start_angle)]) + offset
                end = np.array(
                    [r*np.cos(end_angle), r*np.sin(end_angle)]) + offset
                # if colliding with previous test cases
                if_collide = False
                for j in range(i):
                    radius_start = test_case[j, 5] + \
...
if __name__ == '__main__':
    ...
    for i in range(num_test_cases):
        ...
        test_case = generate_rand_test_case_multi(
            num_agents, side_length, speed_bnds, radius_bnds, is_end_near_bnd=False, is_static=False)
        test_cases.append(test_case)
    # print test_cases

    pickle.dump(test_cases, open(
        "/home/mfe/ford_ws/src/2017-avrl/src/environment/Collision-Avoidance/test_cases/%s_agents_%i_cases.p" % ("2_3_4", num_test_cases), "wb"))
